...
export default function GraphCard({ dirName, title, nodes, edges, image, source, className = "", children, ...props }) {
    return (
        <div className="min-h-full block px-[1rem] py-[0.7rem] text-start border border-[rgba(255,255,255,0.4)]  rounded-[1rem]">
            ...
            <NavLink to={`visualize/${dirName}`} className={"block text-center mt-[1rem] mb-[0.2rem] p-[0.5rem] w-full border hover:bg-green-500 hover:text-black transition border-green-500 rounded-[0.5rem]" }>Explore</NavLink>
        </div>
    );
}