...
public class Louvain {
    ...
    public static List<Map<Integer, Set<Integer>>> louvain(Graph graph, String directory) {
        ...
        // dendrogram maps each super node in level i to the set of nodes in level i-1
        // that were aggregated to become that super node
        ...
        while (canImprove) {// keep iterating until modularity no longer increases
            ...
            double optimizedModularity = modularityOf(graph, optimizedCommunities);// compute modularity after community
                                                                                   // optimization
            // for visualization create 2 JSON files, before and after optimization. create
            // them once then use the JSONs as needed
    ...
    public static Partition optimize(Graph graph, Partition communities) {
         ...
         * 3- calculate modularity gain for each neighboring community (including the
         * original one)
         * 4- select the community with the highest modularity gain
         * 5- insert the node into the selected community
         * 6- repeat steps 1-5 until nodes are always inserted back to their original
        ...
        while (canimprove) {// keep iterating until nodes aren't moving between communities anymore, meaning
            ...
            for (Node node : graph.getNodes()) {// 1- select a node
                ...
                // modularity gain = connectionWeight - (degreeOfNode * degreeOfCommunity) / (2
                // * graphWeight)
                for (int candidateCommunity : neighborCommunities.keySet()) {// for each neighbor community
                    ...
                    double newModularityGain = communities.computeModularityGain(node, candidateCommunity,
                            connectionWeight);
    ...
    private static Map<Integer, Integer> getCommunityToSuperNodeId(Partition communities) {
        ...
        for (int i = 0; i < communities.getNodeToCommunity().length; i++) {// for each community
            if (communities.getCommunitySizes()[i] == 0)
    ...
    // turns every non-empty community to a super node, and returns the map for
    // community -> superNode
    private static Map<Integer, Node> createSuperNodes(Graph aggregatedGraph, Partition communities) {
        ...
        for (int i = 0; i < communities.getNodeToCommunity().length; i++) {
            if (communities.sizeOf(i) == 0)
                continue;// if the community is not empty then it is converted to a super vertex
            ...
            if (communityWeight > 0)
                aggregatedGraph.addEdge(new Edge(superNode, superNode, communityWeight));
        }
        return communityToSuperNode;
    }

    // always returns the same id for a connection between two communities,
